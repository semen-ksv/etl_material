...
def get_material_type(material_type_df: pd.DataFrame, material_id: pd.Series, row_name: str) -> str:
    ...
    return material_type_row.values[0] if material_type_row.values else np.nan
...
def get_unit_measure(material_type_df, unit_of_measure_df, material_id: pd.Series, row_name: str) -> str:
    ...
    if material_unit_value is not np.nan:
        unit_measure_row = unit_of_measure_df.loc[unit_of_measure_df.name == material_unit_value, row_name]
        return unit_measure_row.values[0] if unit_measure_row.values else np.nan
    else:
...
def denormalize_data() -> bool:
    ...
    chunk_list = []  # save all data frame of Materials after demoralization
    if material_df:
